/**
 * Operational / financial ERP data for one tenant company (inventory, orders, collections,
 * settlements, expenses, payroll, supplier ledger).
 * Preconditions: Company, admin user, medical-rep pool (for orders & payroll collections) exist.
 */
const mongoose = require('mongoose');
const { roundPKR } = require('../../../src/utils/currency');
const { getNextSequenceNumber } = require('../../../src/utils/orderNumber');
const {
  Product,
  Distributor,
...
async function seedCompanyOperationalBundle({ rng, code, index, company, admin, reps, dimensions = {} }) {
  const scale = {
    productCount: 24,
    distributorCount: 7,
    pharmacyCount: 10,
    stockTransferCount: 55,
    orderDays: 60,
    collectionAttempts: 90,
    expenseCount: 36,
    supplierPaymentCount: 12,
    ...dimensions
  };
  ...
  const products = [];
  for (let i = 0; i < scale.productCount; i += 1) {
  ...
  const distributors = [];
  for (let i = 1; i <= scale.distributorCount; i += 1) {
  ...
  const pharmacies = [];
  for (let i = 1; i <= scale.pharmacyCount; i += 1) {
    pharmacies.push(
  ...
  // Stock transfers (dates spread over order window)
  for (let t = 0; t < scale.stockTransferCount; t += 1) {
    const daysAgo = randInt(rng, 1, Math.max(7, scale.orderDays));
  ...
  const ordersPerDay = [];
  for (let d = 0; d < scale.orderDays; d += 1) {
  ...
  for (const day of ordersPerDay) {
    for (let i = 0; i < day.count; i += 1) {
      ...
      const orderNumber = await getNextSequenceNumber(company._id, `O${code}`);
  ...
  // Collections with FIFO allocation style (attempts capped; some may skip)
  for (let i = 0; i < scale.collectionAttempts; i += 1) {
    const pharmacy = pick(rng, pharmacies);
    ...
    const collectionDate = dateWithRng(rng, randInt(rng, 1, Math.max(3, scale.orderDays - 2)), 12);
  ...
  const expenseRows = [];
  for (let i = 0; i < scale.expenseCount; i += 1) {
    ...
    const date = dateWithRng(rng, randInt(rng, 1, Math.max(7, scale.orderDays - 2)), 13);
  ...
  let paymentTotal = 0;
  for (let i = 0; i < scale.supplierPaymentCount; i += 1) {
  ...
  return {
    ...
    counts: {
      ...
      stockTransfers: scale.stockTransferCount,
      orders: orderCount,
      deliveries: deliveredCount,
      collections: collectionCount,
      expenses: expenseRows.length + reps.length * payrollMonths.length,
      supplierLedger: supplierPurchases.length + scale.supplierPaymentCount,
