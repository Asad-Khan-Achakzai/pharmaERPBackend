...
const importService = require('../services/territoryImport.service');
...
const commit = asyncHandler(async (req, res) => {
  const result = await importService.commitWorkbook(
    ...
    req.body.fileBase64,
    req.body.mapping,
    { sheet: req.body.sheet, skipExisting: req.body.skipExisting }
  );
...
const template = asyncHandler(async (_req, res) => {
  ...
  res.setHeader('Content-Disposition', 'attachment; filename="territory-import-template.xlsx"');
