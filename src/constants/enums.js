...
/** User-facing distributor handover of company-share cash (not a direct C→D settlement). */
const REMITTANCE_KIND = {
  COLLECT_AND_REMIT: 'COLLECT_AND_REMIT',
  REMIT_EXISTING: 'REMIT_EXISTING'
};
...
const REMITTANCE_DIFFERENCE_REASON = {
  COLLECTION_DISCREPANCY: 'COLLECTION_DISCREPANCY',
...
const LEDGER_REFERENCE_TYPE = {
  ...
  AMENDMENT_CLEARING_ADJ: 'AMENDMENT_CLEARING_ADJ',
...
/** Post-delivery order amendment classification (v1: QUANTITY_REDUCTION only). */
...
/** Customer-facing Credit Note linked to an OrderAmendment (thin identity). */
...
/** Company attendance mode — LEGACY is default for all existing tenants. */
const ATTENDANCE_SYSTEM_MODE = {
  LEGACY: 'LEGACY',
  CHECKIN_POLICY_V2: 'CHECKIN_POLICY_V2'
};

/** Resolved check-in zone evaluation (V2 only; never blocks attendance). */
const ATTENDANCE_LOCATION_STATUS = {
  WITHIN_ZONE: 'WITHIN_ZONE',
  OUT_OF_ZONE: 'OUT_OF_ZONE'
};

/** Weekly plan check-in override types (V2 only). */
const CHECKIN_POLICY_TYPE = {
  COMPANY_DEFAULT: 'COMPANY_DEFAULT',
  FIRST_PLANNED_VISIT: 'FIRST_PLANNED_VISIT',
...
/** Product catalog taxonomy tree (Therapy → Area → Class). */
const PRODUCT_TAXONOMY_KIND = {
  THERAPY: 'THERAPY',
  AREA: 'AREA',
  CLASS: 'CLASS'
};

const PRODUCT_TAXONOMY_PARENT_KIND = {
  THERAPY: null,
  AREA: PRODUCT_TAXONOMY_KIND.THERAPY,
  CLASS: PRODUCT_TAXONOMY_KIND.AREA
};
...
module.exports = {
  ...
  PRODUCT_TAXONOMY_KIND,
  PRODUCT_TAXONOMY_PARENT_KIND,
